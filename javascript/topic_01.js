...
function clearCell(sectionname) {
   let cellContents = `<div></div>`;
   document.getElementById(sectionname).innerHTML = cellContents;
}

// Good ole fashion "For Loop"
function forLoopTest() {
   // TODO: Create a loop that will pause each iteration
...
function doLoopTest() {
   ...
   let end = doEInput == '' ? 0 : doEInput.value;
   // create variable to hold final output of the loop
   let finalText = '';
   //    it's valid input. (no negative inputs)
   if (end < 0) {
